...
1. Drop the full `OFL.txt` next to any font shipped under the OFL — this repo
   does not currently carry one, because the placeholder was copied from a
   vendored tree that did not include it.
2. Decide on the real UI face (see AGENTS.md §5). Candidate rounded, openly
   ...
   **Nunito**, **M PLUS Rounded 1c** (the last also carries broad CJK coverage,
   which matters for §5's Unicode note).
3. Check codepoint coverage against real post content. Neither Lato nor most
